...
OUTPUT=STAGE/'TestResults/CharacterPipeline/RigReview/StellaLegIntegrated04'
...
for side in ['Left','Right']:
    ...
    pco,ptri,_=mesh_arrays(patch);grid=pco.reshape(97,96,3);centers=grid.mean(1)
    ...
    for row in range(1,96):
        for col in range(96):
            ...
            vi=vertex((side,row,col),p,wrow);patchids[(row,col)]=vi;pnorm[vi]=pn[row*96+col];param[vi]=(col/96,row/96)
    for end in [0,1]:
    ...
    bridge([patchids[(95,c)] for c in range(96)],outer_loops[(side,1)])
