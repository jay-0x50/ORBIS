...
namespace Orbis.Game.Editor
{
    ...
    public static class StellaLegCandidateImport
    {
        public const string CandidateRoot=CharacterPipelineImport.Root+"/StellaIntegrated06";
        ...
        {
            Import();
            StellaLegCandidatePreview.CaptureRequested();
        }
        public static void Import()
        {
            string modelPath=Folder+"/Stella.fbx",reportPath="TestResults/CharacterPipeline/UnityImport/StellaIntegrated06/TwoMaterial.json";
            ...
            try
            {
                ...
                File.WriteAllText(reportPath,JsonUtility.ToJson(new Report {modelSha256=Hash(modelPath),prefab=Prefab,avatar=AssetDatabase.GetAssetPath(animator.avatar),
                    ...
                    note="Review candidate only. Existing live roster, V3 candidates, controllers and shared shaders untouched. Mask G=roughness/B=metallic is retained as linear data; the common toon currently has no Mask input, so Blender PBR roughness is not claimed reproduced. BentLegs seam and KneeFlex source-thigh folds remain under actual URP review."},true));
            }
        ...
        public static Dictionary<string,string> ProtectedSnapshot()
        {
            ...
            foreach(string root in new[]{"Assets/Orbis/Game/Characters/Candidates/OriginTrial01","Assets/Orbis/Game/Characters/Candidates/Polaris","Assets/Orbis/Game/LookDev/Shaders"})
