...
namespace Orbis.Game.Editor
{
    ...
    public static class StellaLegCandidate12Preview
    {
        ...
        [Serializable] sealed class Evidence
        {
            ...
            public Vector3 camera,target,keyRotation,keyForward;public Color keyColor;public float fieldOfView,keyIntensity,keyYaw;public bool outline,keyShadows;
        }
        sealed class Rest
        {
        ...
        {
            string run=Argument("-stellaLegRun")??"Integrated12Toon01";
            float keyYaw=-35f;string keyYawText=Argument("-stellaKeyYaw");
            if(keyYawText!=null&&(!float.TryParse(keyYawText,System.Globalization.NumberStyles.Float,System.Globalization.CultureInfo.InvariantCulture,out keyYaw)||float.IsNaN(keyYaw)||float.IsInfinity(keyYaw)))throw new ArgumentException("-stellaKeyYaw requires a finite invariant-culture degree value.");
            ...
            string poseFile="Tools/CharacterPipeline/PendingImport/StellaLegIntegrated12/ReviewPoses.json";
            var source=JsonUtility.FromJson<Source>(File.ReadAllText(poseFile));
            var guard=StellaLegCandidate12Import.ProtectedSnapshot();string modelHash=StellaLegCandidate12Import.Hash(StellaLegCandidate12Import.Folder+"/Stella.fbx");
            ...
            var model=Object.Instantiate(AssetDatabase.LoadAssetAtPath<GameObject>(StellaLegCandidate12Import.Prefab),host.transform);
            ...
            light.shadows=LightShadows.Soft;light.transform.rotation=Quaternion.Euler(42,keyYaw,0);RenderSettings.sun=light;
            ...
            try
            {
                ...
                foreach(var view in views)
                {
                    ...
                    foreach(var setting in settings)
                    {
                        ...
                        File.WriteAllText(path+".json",JsonUtility.ToJson(new Evidence{view=view.Item1,pose=view.Item2,prefab=StellaLegCandidate12Import.Prefab,modelSha256=modelHash,
                            ...
                            keyRotation=light.transform.eulerAngles,keyForward=light.transform.forward,keyColor=light.color,keyIntensity=light.intensity,keyYaw=keyYaw,
            ...
            finally
            {
                ...
                foreach(var material in materials)Object.DestroyImmediate(material);StellaLegCandidate12Import.AssertProtected(guard);
            }
            if(StellaLegCandidate12Import.Hash(StellaLegCandidate12Import.Folder+"/Stella.fbx")!=modelHash)throw new InvalidOperationException("Preview changed source FBX");
