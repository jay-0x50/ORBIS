# Runs all EditMode and PlayMode tests, including the completed M0 regression suite.
param(
...
function Invoke-M1Unity([string[]] $AdditionalArguments, [string] $LogName) {
...
Invoke-M1Unity @('-quit', '-executeMethod', 'Orbis.M1.Editor.M1ProjectSetup.SetupAndValidate') 'M1-setup.log'
foreach ($mode in @('EditMode', 'PlayMode')) {
    $resultPath = Join-Path $resultsRoot ('M1-' + $mode + '.xml')
    # A stale success report must not make a failed run appear successful.
    $startedAt = [DateTime]::UtcNow
    Invoke-M1Unity @('-runTests', '-testPlatform', $mode,
        '-testResults', ('"' + $resultPath + '"')) ('M1-' + $mode + '.log')
